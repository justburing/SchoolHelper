 ...
 */
public class ReceiverExpressDetailActivity extends BaseActivity {
    ...
    public void onClick(View view) {
        switch (view.getId()) {
            case R.id.backLL:
                ...
                break;
            case R.id.chat:
                startActivity(new Intent(ReceiverExpressDetailActivity.this, ChatActivity.class)
                ...
                .subscribe(new Subscriber<MyExpressDetailBean>() {
                    ...
                    public void onNext(MyExpressDetailBean myExpressDetailBean) {
                        ...
                        if (myExpressDetailBean != null) {
                            Glide.with(ReceiverExpressDetailActivity.this).load(myExpressDetailBean.getPublisher().getAvatar()).into(publisherAvatar);
        ...
        RetrofitUtil.getRetrofitApiInstance().comfirmDelivery(userInfoBean.getToken(), id)
                ...
                .subscribe(new Subscriber<JSONObject>() {
                    ...
                    public void onError(Throwable e) {
                        AndroidFragUtil.dismissDialog(getSupportFragmentManager());
                        if (e instanceof MalformedJsonException) {
                            Toast.makeText(ReceiverExpressDetailActivity.this, "确认送达快递失败", Toast.LENGTH_SHORT).show();
                        } else if (e instanceof HttpException) {
                            try {
                                JSONObject object = JSON.parseObject(((HttpException) e).response().errorBody().string(), JSONObject.class);
                                Toast.makeText(ReceiverExpressDetailActivity.this, object.getJSONObject("error").getString("message"), Toast.LENGTH_SHORT).show();
                            } catch (IOException e1) {
                                e1.printStackTrace();
                            }
                        } else if (e instanceof ConnectException) {
                            ...
                            Toast.makeText(ReceiverExpressDetailActivity.this, e.toString(), Toast.LENGTH_SHORT).show();
                        }
                        Log.e("error", e.toString());
                    }
                    ...
                    public void onNext(JSONObject jsonObject) {
                        AndroidFragUtil.dismissDialog(getSupportFragmentManager());
                        Toast.makeText(ReceiverExpressDetailActivity.this, "确认成功", Toast.LENGTH_SHORT).show();
                        comfirmFinish.setBackgroundColor(R.color.red300);
                        comfirmFinish.setEnabled(false);
                    }
                });
